...
/*--------------------------------------------------------------------------
	...
	-	usage
		...
		newCornersObj =
		new curvyCorners(settingsObj,
			divObj1[, divObj2[, divObj3[, . . . [, divObjN]]]]);
--------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------
	-	description
			This method creates the corners and applies them to the div
			element.
	-	params
	-	return
--------------------------------------------------------------------------*/
curvyObject.prototype.applyCorners = function()
{
	...
	for(var i in corners)
	{
		...
		if(!this.settings[cc])
		{
			...
			if(((cc == "tr" || cc == "tl") && this.topContainer != null) || ((cc == "br" || cc == "bl") && this.bottomContainer != null))
			{
				...
				{
					oSystem_Colors.SetBackground(newCorner, this.settings.colorType);
				}
				else
				{
			...
			for(z in radiusDiff)
			{
				...
				if(z == "t" || z == "b")
				{
					if(radiusDiff[z])
					{
						...
						oSystem_Colors.SetBackground(newFiller, this.settings.colorType);
					...
					oSystem_Colors.SetBackground(newFillerBar, this.settings.colorType);
...
curvyObject.prototype.drawPixel = function(intx, inty, colour, transAmount, height, newCorner, image, cornerRadius)
{
	...
	{
		oSystem_Colors.SetBackground(pixel, this.settings.colorType);
	}
...
/*--------------------------------------------------------------------------
	-	description
			Returns index if the passed value is found in the array
