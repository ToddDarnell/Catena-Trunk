...
oDCR_SearchDetails = new system_AppScreen();

oDCR_SearchDetails.screen_Name = "document_DCRSearchDetails";
oDCR_SearchDetails.screen_Menu = "DCR Details";
oDCR_SearchDetails.module_Name = "dms";

oDCR_SearchDetails.bChangesMade = false;			//	keeps track of whether a request status was modified or not

oDCR_SearchDetails.oRequestTable = new system_SortTable();
oDCR_SearchDetails.oHistoryTable = new system_SortTable();

/*-----------------------------------------------------------------------------
    perform any special cases which need to be performed
    before the screen is displayed
-------------------------------------------------------------------------------*/
oDCR_SearchDetails.custom_Show = function()
{
	oDCR_SearchDetails.ClearFields();
...
*/
//--------------------------------------------------------------------------
oDCR_SearchDetails.LoadLists = function()
{
...
oDCR_SearchDetails.custom_Load = function ()
{
	//this.oUserFilter = new system_UserOrgSelection();
	//this.oUserFilter.load("dcrSearchDetailsActionBox", "dcrSearchDetailsUserPopup");

	this.element_backButton = oScreenBuffer.Get("dcrSearchDetailsBackButton");

	this.element_backButton.backScreen = "";
	this.element_backButton.onclick = function ()
	{
		oCatenaApp.navigate(oDCR_Search.screen_Menu, page_param_display);
	};

	this.requestId = -1;

	this.element_tabs = oScreenBuffer.Get("dcrSearchDetailstab");

	this.tabs = new system_tabManager(this.element_tabs);
	
	this.tabs.addTab(0, "Details", false, SetupDCRSearchDetailsPage);
	this.tabs.addTab(1, "History", false, SetupDCRSearchHistoryPage, ShowDCRSearchHistoryPage);
...
function SetupDCRSearchDetailsPage(element_root)
{
	oDCR_SearchDetails.SetupSearchDetailsPage(element_root);
}

function SetupDCRSearchHistoryPage(element_root)
{
	oDCR_SearchDetails.SetupSearchHistoryPage(element_root);
}
function ShowDCRSearchHistoryPage()
{
	oDCR_SearchDetails.ShowSearchHistoryPage();
}
//--------------------------------------------------------------------------
/**
	\brief
...
*/
//--------------------------------------------------------------------------
oDCR_SearchDetails.SetupSearchDetailsPage = function(element_root)
{	
	element_table = oScreenBuffer.Get("dcrSearchDetailsTable");

	element_root.appendChild(element_table);

	this.oRequestTable.setName("dcrSearchDetailsTable");
	...
	this.oRequestTable.custom_element = dcrSearch_ItemList;

	this.element_DescriptionBox = oScreenBuffer.Get("dcrSearchDetailsNewDocDetails");
	element_root.appendChild(this.element_DescriptionBox);
	
	this.element_DocTitle = oScreenBuffer.Get("dcrSearchDetailsDocTitle");
	this.element_DocType = oScreenBuffer.Get("dcrSearchDetailsDocType");
	this.element_DocVersion = oScreenBuffer.Get("dcrSearchDetailsDocVersion");
	this.element_Request = oScreenBuffer.Get("dcrSearchDetailsRequest");
	this.element_RequestType = oScreenBuffer.Get("dcrSearchDetailsReqType");
	this.element_Status = oScreenBuffer.Get("dcrSearchDetailsStatus");
	this.element_Requester = oScreenBuffer.Get("dcrSearchDetailsRequester");
	this.element_Model = oScreenBuffer.Get("dcrSearchDetailsReqModel");
	this.element_ModelLabel = oScreenBuffer.Get("dcrSearchDetailsReqModelLabel");
	this.element_Priority = oScreenBuffer.Get("dcrSearchDetailsPriority");
	this.element_Deadline = oScreenBuffer.Get("dcrSearchDetailsDeadline");
	this.element_RQNumber = oScreenBuffer.Get("dcrSearchDetailsRQNumber");
	this.element_Description = oScreenBuffer.Get("dcrSearchDetailsDescription");
	this.element_RequestDate = oScreenBuffer.Get("dcrSearchDetailsRequestDate");
	this.element_AssignedWriter = oScreenBuffer.Get("dcrSearchDetailsAssignedWriter");
}	
...
*/
//--------------------------------------------------------------------------
oDCR_SearchDetails.SetupSearchHistoryPage = function(element_root)
{
	element_table = oScreenBuffer.Get("dcrSearchDetailsHistoryTable");

	element_root.appendChild(element_table);
	
	element_table.style.position = "relative";
		
	this.oHistoryTable.setName("dcrSearchDetailsHistoryTable");
...
*/
//--------------------------------------------------------------------------
oDCR_SearchDetails.ShowSearchHistoryPage = function()
{	
	serverValues = null;
    serverValues = Object();
	...
	server.callASync(serverValues, dcrSearchStatusHistory_Parse);
}
...
*/
//--------------------------------------------------------------------------
function dcrSearchStatusHistory_Parse(strResults)
{
    oDCR_SearchDetails.oHistoryTable.SetList(strResults);
    oDCR_SearchDetails.oHistoryTable.Display();
...
*/
//--------------------------------------------------------------------------
oDCR_SearchDetails.ShowUserSelection  = function()
{
	...
	if(this.element_StatusList.value == "Clarify")
	{
		...
		{
			oDCR_SearchDetails.SelectUser(selectedUser);
			return true;
		}
	}
}
...
----------------------------------------------------------------------------*/
oDCR_SearchDetails.ClearFields = function()
{
...
----------------------------------------------------------------------------*/
/*oDCR_SearchDetails.ActionSubmit = function()
{		
...
/*--------------------------------------------------------------------------
	...
	or unselected.
----------------------------------------------------------------------------*/
/*oDCR_SearchDetails.SelectUser = function(userName)
{
	/*
	...
	*/
	...
	{
		this.arUser = null;
		screen_SetTextElement(this.element_ClarifyUserSelected, "");	
	}
...
----------------------------------------------------------------------------*/
oDCR_SearchDetails.Display = function(requestId, reqType)
{
	/*
...
*/
oDCR_SearchDetails.AddElement = function(currentStep, requestUpdate, pageNumber, stepNumber)
{
